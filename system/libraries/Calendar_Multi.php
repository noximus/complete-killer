...
/**
 * CodeIgniter Calendar Class - Modified For Multiple Date Entries
 *
...
class CI_Calendar_Multi {
	 ...
	 * @access	public
	 */
	function CI_Calendar_Multi($config = array())
	{		
		...
		while ($day <= $total_days)
		{
			...
			for ($i = 0; $i < 7; $i++)
			{
				...
				{ 					
					if (isset($data[$day]))
					{	
						$out .= "<strong>".$day."</strong>";
						if(is_array($data[$day]))
						{
							//Cells with multiple contents
							foreach($data[$day] as $value)
							{
								$temp = ($is_current_month == TRUE AND $day == $cur_day) ? $this->temp['cal_cell_content_today'] : $this->temp['cal_cell_content'];
								$out .= str_replace('{content}', $value, $temp);
							}
						}
						else
						{
							// Cells with content
							$temp = ($is_current_month == TRUE AND $day == $cur_day) ? $this->temp['cal_cell_content_today'] : $this->temp['cal_cell_content'];
							$out .= str_replace('{content}', $data[$day], $temp);
						}
					}
					else
					{
